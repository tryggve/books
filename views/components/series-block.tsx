...
const seriesChevron = css`width: 12px; height: 12px; transform: rotate(90deg); transition: transform 0.2s; color: var(--color-text-tertiary);`
const seriesChevronClosed = css`transform: rotate(0deg);`
const seriesChildren = css`display: block;`
const seriesChildrenClosed = css`display: none!important;`
    ...
    return (
        <div class={seriesBlock}>
            <div class={seriesRow} data-component='series-row'>
                <svg class={seriesChevron} data-class-toggle={seriesChevronClosed} viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="5,3 9,7 5,11" /></svg>
                <span class={seriesName}>{name}</span>
                <span class={seriesCount}>{count} {count !== 1 ? 'böcker' : 'bok'}</span>
            </div>
            <div class={seriesChildren} data-class-toggle={seriesChildrenClosed}>
                {children}
            </div>
        </div>
    )
